...
public class CLAN_COMMISSION_STAFF_REQ extends game.network.game.GamePacketREQ
{
	...
	public void runImpl()
	{
				...
				for (long pId : list)
				{
					try
					{
						for (Player f : ck.getPlayers(c))
						{
							...
							{
								f.role = ClanRole.CLAN_MEMBER_LEVEL_STAFF;
		...
		client.sendPacket(new CLAN_COMMISSION_STAFF_ACK(error));
	}
	public void sendMessage(Clan c, Player p) throws Exception
	{
		if (c != null && p != null)
		{
			PlayerMessage msg = new PlayerMessage(p.id, p.id, 15, c.id, NoteType.WEB, NoteReceive.AUXILIAR_CLAN, false);
			msg.name = c.name;
