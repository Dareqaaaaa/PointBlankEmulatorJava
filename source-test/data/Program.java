...
import game.network.game.GamePacketIdACK;
...
import core.config.dat.TitlesDAT;
import core.enums.ShopTag;
import core.info.Software;
import core.model.Good;
import core.model.Maps;
import core.model.PlayerInventory;
import core.model.RankInfo;
import core.model.Room;
import core.model.TitleV;
import core.model.User;
import core.postgres.sql.DatabaseFactory;
import core.postgres.sql.PlayerSQL;
import core.utils.*;
...
public class Program
{
	...
	public static void main(String[] args) throws NoSuchAlgorithmException, IOException, ParseException, InterruptedException
	{
		...
		//Software.printIntro("", "DATA", Color.PINK);
		...
		while (true)
		{
			try
			{
				...
				else if (comando.startsWith("patch_user"))
				{
					/*int sucesso = 0;
					List<User> list = new ArrayList<User>();
					list.add(new User("pedro42", "1014", "145.14.134.71",  "1.15.42",  38, ""));
			    	BinaryWriter bw = new BinaryWriter("users.pistola");
			    	bw.WriteD(list.size());
			    	for (User c : list)
			    	{
			    		bw.WriteS(c.user);
			    		...
			    		bw.WriteS(c.host);
			    		bw.WriteS(c.client);
			    		bw.WriteD(c.revision);
			    		bw.WriteS(c.hwid);
			    		sucesso++;
			    	}
			    	System.out.println(" Path Users: " + sucesso + " sucesso.");
			    	bw.flush();
			    	bw.close();*/
				}
				else if (comando.startsWith("enc "))
				{
					...
					enc.encryptionKey = "ABCDEFGHIJKLMNO";
					enc.aesEncryptionAlgorithem = "AES";
					System.out.println(" Texto Encryptado: " + enc.enc(texto));
				}
				else if (comando.startsWith("dec "))
				{
					...
					enc.encryptionKey = "ABCDEFGHIJKLMNO";
					enc.aesEncryptionAlgorithem = "AES";
					System.out.println(" Texto Decryptado: " + enc.dec(texto));
				}
				...
				else if (comando.startsWith("key "))
				{
					String texto = comando.substring(4);
					//EncryptSyncer enc = EncryptSyncer.gI();
					//enc.aesEncryptionAlgorithem = texto;
					System.out.println(" Algoritimo Definido: " + texto);
				}
				else if (comando.startsWith("title"))
				{
					long value = 0;
					TitlesDAT td = TitlesDAT.gI();
					for (int i = 1; i < 45; i++)
					{
						TitleV t = td.titles.get(i);
						if (t != null)
						{
							//value += t.pos_v;
						}
					}
					//System.out.println("Mask Title 1: " + value);
					//System.out.flush();
					value = 0;
					for (int i = 1; i < 45; i++)
					{
					...
					System.out.println("Mask Title 2: " + value);
					System.out.flush();
				}
				else if (comando.startsWith("date"))
				{
					Integer count = 1809130000;
					String oldDate = count.toString();
					...
					int newDate = Integer.parseInt(new SimpleDateFormat("yyMMddHHmm").format(cal.getTime()));
					System.out.println("Data antiga: " + oldDate);
					System.out.println("Data nova: " + newDate);
					System.out.flush();
				}
				else if (comando.startsWith("opcode"))
				{
					List<String> list = new ArrayList<String>();
					for (File f : new File("packets").listFiles())
					{
					...
					{
						game.network.game.GamePacketREQ packet = null;
						switch (i)
						{
						...
						if (packet != null)
						{
							System.out.println("name: " + packet.getName());
							if (!list.contains(packet.getName()))
							{
								System.out.println("Pacote não listado: " + packet.getName());
								System.out.flush();
							}
							packet = null;
						}
				...
				else if (comando.startsWith("mapteste"))
				{
					long position = 0;
				...
				else if (comando.startsWith("teste"))
				{
					List<Long> list = new ArrayList<Long>();
					for (int i = 0; i < 100000; i++)
						list.add((long) (i * 89884));
					...
					//...
					start = System.currentTimeMillis();
					list.forEach(i -> { long get = i; });
					end = System.currentTimeMillis();
					System.out.println("ForEach: " + (end - start));
					
					//...
					start = System.currentTimeMillis();
					list.stream().forEach(i -> { long get = i; });
					end = System.currentTimeMillis();
					System.out.println("ForEach Stream: " + (end - start));
					
					//...
					start = System.currentTimeMillis();
					list.stream().forEachOrdered(i -> { long get = i; });
					end = System.currentTimeMillis();
					System.out.println("ForEach Stream Ordered: " + (end - start));
					...
					System.out.println("While Interator: " + (end - start));
					...
					System.out.println("While For: " + (end - start));
				...
				else
				{
				    ...
				    System.out.println(" MD5: " + hexFileCSV);*/
